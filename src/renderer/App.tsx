...
type ToolId = "codec" | "crypto" | "time" | "cron" | "regex" | "json" | "markdown" | "jwt";
  ...
  const inventoryCommands = useMemo<Command[]>(() => {
    ...
    const actions: Command[] = [
      ...
      },
      {
        ...
        keywords:
          "tools codec crypto time cron regex json markdown jwt hash base64 image aes rsa ecdsa md5 timestamp unix timezone schedule regexp match replace format minify pretty beautify mermaid flowchart preview",
  ...
  const toolsCommands = useMemo<Command[]>(() => {
    ...
    const actions: Command[] = [
      ...
      },
      {
        ...
        icon: <Wrench className="h-4 w-4" />,
        keywords:
          "tools codec crypto time cron regex json markdown jwt hash base64 image aes rsa ecdsa md5 timestamp unix timezone schedule regexp match replace format minify pretty beautify mermaid flowchart preview token decode verify",
        run: () => handleModeChange("tools"),
      },
      {
        id: "mode-flow",
