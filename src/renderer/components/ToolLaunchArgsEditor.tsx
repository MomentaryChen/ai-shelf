  ...
  return (
    <div className={compact ? "flex flex-col gap-2" : "flex flex-col gap-2.5"}>
      {INVENTORY_TOOL_IDS.map((tool) => (
        <label
          key={tool}
          className="flex flex-col gap-1 rounded-lg border border-border px-3 py-2.5 sm:flex-row sm:items-center sm:gap-3"
        >
          ...
          <input
            ...
            spellCheck={false}
            className="min-w-0 flex-1 rounded-md border border-border bg-bg-secondary px-3 py-1.5 font-mono text-[12px] text-text-primary placeholder:text-text-tertiary focus:border-accent/40 focus:outline-none"
          />
        </label>
      ))}
    </div>
