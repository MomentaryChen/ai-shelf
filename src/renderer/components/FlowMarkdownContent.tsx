...
function isMermaidCode(child: unknown): child is ReactElement<{
  className?: string;
  children?: ReactNode;
}> {
  return (
    ...
    String((child.props as { className?: string }).className ?? "").includes(
      "language-mermaid",
    )
  );
}
  ...
  return (
    ...
    >
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="mb-3 mt-5 text-[18px] font-semibold text-text-primary first:mt-0">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="mb-2 mt-4 text-[16px] font-semibold text-text-primary first:mt-0">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="mb-2 mt-3 text-[15px] font-medium text-text-primary first:mt-0">
              {children}
            </h3>
          ),
          ...
          code: ({ className: codeClass, children }) => {
            ...
            if (isBlock) {
              return (
                <code
                  className={`${codeClass ?? ""} block overflow-x-auto rounded-[12px] bg-bg-elevated/60 px-3 py-2 font-mono text-[12px]`}
                >
                  {children}
                </code>
              );
          ...
          pre: ({ children }) => {
            ...
            if (isMermaidCode(only)) {
              const chart = String(only.props.children ?? "").replace(/\n$/, "");
