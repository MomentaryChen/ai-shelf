...
The desktop app has **four** top-level modes in the header:
...
While **Inventory** mode is open, the window footer shows **🔄 Refresh** to re-run detection and reload inventory tabs (Usage has its own refresh controls).
...
![Terminal](../tests/screenshots/en/02.terminal.png)
...
![Models](../tests/screenshots/en/03.models.png)
...
![Skills](../tests/screenshots/en/04.skills.png)
...
![MCP](../tests/screenshots/en/05.mcp.png)
...
![Config](../tests/screenshots/en/06.config.png)
...
![Doctor](../tests/screenshots/en/07.doctor.png)
...
![Update](../tests/screenshots/en/08.update.png)
...
**Usage** is an Inventory left-rail section (no dedicated screenshot asset yet). It aggregates spend and budgets across supported paid CLI providers so you can decide where cost goes—not only see totals.

Typical capabilities:

- Daily rollups and provider breakdowns (Claude, Codex, Cursor, Gemini, Copilot where credentials allow)
- Weekly budget targets and attribution back to Flow authoring / runs when applicable
- Credentials stored via OS `safeStorage` when available; some providers need API keys or account cookies configured in-app

Open **Inventory → Usage** from the header, or jump via the command palette.

---

## 10. Tools

**Tools** is a top-level mode (left rail of utility panels). No dedicated screenshot assets yet; regenerate later via `pnpm gen:docs-assets` when capture coverage expands.

| Panel | Purpose |
|---|---|
| **Codec** | Hash / encode / decode / image Base64 |
| **Crypto** | AES / RSA / ECDSA helpers |
| **Time** | Timestamp and timezone conversion |
...
**AI Flow** is a top-level mode for multi-step agent automation. No dedicated screenshot assets yet.

What you can do:

- Browse and install **flow templates**, or create a flow via in-app chat (drafts a `.flow.md`)
- Inspect the **DAG**, runner settings, schedule (cron), and next run time
- Run flows manually; stream agent console output while a run is in progress
- Review **run history** (status, duration, progress, events) and open run artifacts
- Prefer the in-app scheduler while the app is open; on Windows, OS Task Scheduler can call `ai-shelf flow due` when the app is closed (see [ai-flow-phase1.md](ai-flow-phase1.md))

Flows live under the app data `flows/` folder; format details are in [flow-format.md](flow-format.md).

---

## Notes

- Screenshot order and filenames for Overview–Update are generated by `tests/e2e/screenshot.spec.ts` into `tests/screenshots/en/` (this page) and `tests/screenshots/zh/` ([pages.zh-TW.md](pages.zh-TW.md)). Usage / Tools / Flow sections above are documented without PNGs until capture coverage expands.
- Regenerate all README/docs images before a release: `pnpm gen:docs-assets` on Windows (see [RELEASE.md](RELEASE.md)).
